...
public abstract class MiniMaxAgent : GameAgent
{
    ...
    protected float GetScore(GameAgent player, Position move, int depth = int.MaxValue)
    {
        ...
        else
        {
            List<Position> possibleMoves = game.GetPossibleMoves();
            for (int i = 0; i < possibleMoves.Count; ++i)
                score = Mathf.Min(score, -GetScore(player.opponent, possibleMoves[i], depth - 1));
        }
